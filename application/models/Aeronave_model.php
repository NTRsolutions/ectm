...
class Aeronave_model extends CI_Model {
    ...
    //-- get prefixo
    public function check_nome($idempresa,$prefixo){
    	$this->db->select('AER_PREFIXO');
    	$this->db->from('tbaeronave');
    	$this->db->where('AER_EMPRESA =', $idempresa);
    	$this->db->where('AER_PREFIXO', $prefixo);
    	$this->db->limit(1);
    	$query = $this->db->get();
    	if($query->num_rows() == 1) {
    		return $query->result();
    ...
    //-- get aeronaves
    function get_all_aeronaves($idempresa=null){
        $this->db->select('*');
        $this->db->from('tbaeronave');
        $this->db->where('AER_EMPRESA =', $idempresa);
        $this->db->order_by('AER_ID', 'DESC');
        $query = $this->db->get();
        $query = $query->result_array();  
        return $query;
    } 
